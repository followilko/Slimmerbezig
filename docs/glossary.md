...
| **Suggested** | UI label for the personalised feed at **`/for-you`**. Full-viewport **depth tiles** GSAP carousel ([`components/feed/depth-tiles.tsx`](../components/feed/depth-tiles.tsx) + [`suggested-depth-feed.tsx`](../components/feed/suggested-depth-feed.tsx)) cycling a subset (**`SUGGESTED_TILE_COUNT` = 6**) of recommended **`PostCard`**s from `get_recommended_hacks` v2 (with a recent-published fallback when no tag overlap). Falls back to a simple grid when fewer than two posts. **`/explore`** keeps the scannable multi-column grid. View tracking is off in the carousel (all tiles are technically on-screen); reduced-motion users see a single static card. See ADR 2026-06-05 — Suggested depth tiles carousel. |
