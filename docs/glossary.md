...
| **Suggested** | UI label for the personalised feed at **`/for-you`**. Renders `get_recommended_hacks` v2 (with a recent-published fallback when no tag overlap) decorated by **`POST_META_BY_ID`** in **`lib/dummy/posts.ts`**. The 10 seeded posts are real `hacks` rows from **`supabase/08_seed_dummy_posts.sql`**; TS metadata (post type, minutes, author block, peers, metrics) stays in code until the B2B schema deltas (`post_type`, structured title columns, praise/points) land. See ADR 2026-05-28 — Dummy posts seeded as curated hacks. |
